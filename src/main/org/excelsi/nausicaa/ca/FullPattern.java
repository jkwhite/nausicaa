...
public class FullPattern {
    ...
    public FullPattern(int[] pattern, int dims) {
        _p = pattern;
        _dims = dims;
    }
    ...
    public FullPattern mirror() {
        int[] m = new int[_p.length];
        for(int i=0;i<_p.length-1;i++) {
            m[m.length-2-i] = _p[i];
        }
        m[m.length-1] = _p[_p.length-1];
        return new FullPattern(m, _dims);
    }

    public Iterator<FullPattern> mirrors() {
        switch(_dims) {
            case 1:
                return Arrays.asList(new FullPattern[]{mirror()}).iterator();
            case 2:
                final FullPattern[] ps = new FullPattern[3];
                return new Iterator<FullPattern>() {
                    int[] start = _p;
                    int i=0;

                    public FullPattern next() {
                        int[] next = rotate(start);
                        ps[i] = new FullPattern(next, _dims);
                        start = next;
                        FullPattern ret = ps[i];
                        i++;
