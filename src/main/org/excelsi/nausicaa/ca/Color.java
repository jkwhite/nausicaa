...
public class Color extends AbstractMutator {
    ...
    @Override public IndexedRule mutateIndexedRule(IndexedRule r) throws MutationFailedException {
        ...
        if(a.colors()>8) {
            ...
            IndexedRule nr = (IndexedRule) r.origin().derive(a.asColors(a.colors()+1), r);
            nr = new ThickenOne((byte)(nr.getPattern().archetype().colors()-1)).mutateIndexedRule(nr);
            return nr;
        }
    }
    ...
    public Multirule mutate(Multirule r) {
        ...
        for(int j=0;j<rules.length;j++) {
            if(j!=i) {
                ...
                nr[j] = rules[j].origin().derive(ac, rules[j].length()).random(_om).next();
