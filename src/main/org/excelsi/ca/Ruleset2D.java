...
public class Ruleset2D implements Ruleset {
    private int[] _colors;
    private int _length;


    public Ruleset2D(int[] colors) {
        this(colors, 3);
    }

    public Ruleset2D(int[] colors, int length) {
        _colors = colors;
        _length = length*length;
    }

    public Rule create(Object... args) {
        return new Rule2D(this, (int[]) args[0], (int[][]) args[1], (int) ((Integer)args[2]).intValue());
    }

    public Ruleset derive(int[] colors, int len) {
        return new Ruleset2D(colors, len);
    }
    ...
    public Iterator<Rule> iterator() {
        ...
        return new Iterator<Rule>() {
            ...
            public Rule next() {
                ...
                Rule r = new Rule2D(Ruleset2D.this, _colors, pats, _colors[0]);
